...
pub async fn query(
    ...
    let sql = [
        ...
        " AND ($3::bigint   IS NULL OR ts.tournament_submission_id >= $3)",
        " AND ($4::bigint   IS NULL OR ts.tournament_submission_id <= $4)",
        " AND ($5::bigint[] IS NULL OR ts.tournament_submission_id = ANY($5))",
        " AND ($6::bigint[] IS NULL OR ts.creator_user_id = ANY($6))",
        " AND ($7::bigint[] IS NULL OR ts.tournament_id = ANY($7))",
        " AND ($8::bigint[] IS NULL OR ts.submission_id = ANY($8))",
        " AND ($9::bigint   IS NULL OR ts.kind = $9)",
        " ORDER BY ts.tournament_submission_id",
    ]
    .join("\n");
